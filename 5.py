...
async def on_connect():
    print(f"รอกำลังล็อกอินบอท : {bot.user}")
    time.sleep(1.0)
    print("ล็อกอินสำเร็จ")
...
async def sms(ctx, phone, amount:int):
    ...
    if (amount < 160):
      ...
      emBed.add_field(name="จำนวน",value=",value=amount)
      emBed.add_field(name="สนับสนุน",value="0631620915โอนมาได้เลยคับ")
...
@bot.command()
async def help(ctx):
	emBed = discord.Embed(title="Bot BY: บีมคุ",description="วิธิใช้",color=0xff4612)
